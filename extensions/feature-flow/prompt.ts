export function buildKickoffPrompt(input: {
	...
	return [
		...
		"- Possible phases: Strategy → System model → Design → Architecture decisions → Work orders → Execute/report → Review → PR/user guide.",
		"- Design-to-execution matters: create Work Orders from the reviewed system model, decisions, and proof plan, not as generic task churn.",
		...
		"3. If you create strategy/model/decision docs, summarize them and ask the user to review the model before relying on it as execution authority.",
		"4. If docs are unnecessary, keep the strategy and plan in chat and proceed.",
		"5. Work orders are design-derived delegation contracts: include mission, strategic context, code anchors from code_find/semantic_search, minimal-change plan, decisions to preserve, escalation triggers, proof required, and status frontmatter (draft | ready | blocked | done). Review the work-order split with the user before running a loop or executing a ready work order.",
		"",
		"## Hard Gates",
		"- Ask the user to decide before schema, API contract, auth/financial, infra, major dependency, or broad refactor changes.",
