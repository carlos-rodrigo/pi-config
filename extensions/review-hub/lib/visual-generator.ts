  ...
  return `
    <section class="review-section" data-section-id="${escapeAttr(section.id)}">
      <button
        ...
        title="Add comment on: ${escapeAttr(title)}"
        aria-label="Add comment on: ${escapeAttr(title)}"
      >
        ${COMMENT_ICON_SVG}
      </button>
      ${sectionHtml}
