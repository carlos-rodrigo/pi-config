...
- **HTML document mode:** The source HTML is served as the top-level review page, not an iframe. The server strips source scripts/base tags, applies a CSP nonce for the review overlay, preserves normal hash navigation, and exports comments to `<name>.review.md` without modifying the source HTML. Prefer stable `data-review-id` anchors when present. A `data-review-decision="single-choice"` container saves a changed radio choice as anchored feedback. A `data-review-decision="recorded-decision"` container atomically upserts typed decision feedback by stable review anchor: selection edits are `selected`, and checking **Decision recorded** upgrades the same entry to `confirmed` with rationale, owner, completeness, and source fingerprint. Finishing disables the recorders, waits for pending writes, and keeps the review open if persistence fails.
